...
import { downloadCSV, generateCSV, formatCurrencyForExport, formatDateForExport, generateFilename } from "@/lib/export-utils";
...
export default function BookingsPage() {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      setTotalItems(data.pagination?.total || bookingsData.length || 0);
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const rows = dataToExport.map((booking) => ({
        ...
        route: `${booking.schedule.route.departurePort.code} → ${booking.schedule.route.arrivalPort.code}`,
        departure: formatDateForExport(new Date(booking.schedule.departureTime)),
        passengers: booking.passengers.length,
        totalAmount: formatCurrencyForExport(booking.totalAmount),
        status: booking.status,
        paymentStatus: booking.paymentStatus,
        createdAt: formatDateForExport(new Date(booking.createdAt)),
      }));
      ...
      downloadCSV(csv, generateFilename("bookings"));
    } finally {
      ...
      render: (row) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => router.push(`/admin/bookings/${row.id}`)}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Button onClick={handleExport} disabled={isExporting || bookings.length === 0} variant="outline">
